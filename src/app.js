...
const FilterTable = require('./components/filterTable/FilterTable.js');
...
class App extends React.Component{
  ...
  handleSearch(e){
    ...
    const filtered = this.state.data.filter((value)=>{
  ...
  let current = [...this.state.currentData];
    ...
    if(e.target.classList[lastClass] === 'category-index'){
      ...
      let toLargest = current.sort((a,b)=>{
        return a.available_supply - b.available_supply;
      })
  ...
  removeSpaceNumber(number, category){
    if(category === 'price'){
      if(number.charAt(0) === '0'){
        let toNumber = parseFloat(number).toFixed(6);
        return toNumber.replace('.', ',');
      }
      else{
        let toNumber = parseFloat(number).toFixed(2);
        return toNumber.replace('.', ',')
      }
    }
